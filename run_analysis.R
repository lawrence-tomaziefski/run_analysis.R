...
require(stringr)
#========================================================================
# Sets working directory, creates a folder for data, and creates a path to that data.
setwd('/Users/lawrence_tomaziefski/R_Working_Directory/161120_Getting_and_Cleaning_Data')
...
#========================================================================
# Cleaning the Features Labels.
#Read in the features data file to use them as column headers later in the reshaping process.
#In order to make the process of reading the data in easier, I converted the features.txt to a .csv
feature = data.table(read.csv(paste0(path,"features.txt"),header = FALSE,stringsAsFactors = FALSE))
...
duplicate_data = duplicated(feature$feature, fromLast = TRUE) ##returns a logical vector with 84 TRUE
#Use the which function to determine where the duplicates occur
...
feature = mutate(feature,feature=str_replace(feature,"tbody","timebody"))
...
#Determine the length of duplicate filtered so we know exactly how many features measure the mean and std.
howmanymean_std =length(duplicate_filtered) #returns 86, so when go to make our tidy data set of just the
#variables that measure the mean and std we should only have 86.  
#========================================================================
#Cleaning the Activities data
#Read in the features data file to use them as column headers later in the reshaping process.
#In order to make the process of reading the data in easier, I converted the features.txt to a .csv
activities = data.table(read.delim(paste0(path,"activity_labels.txt"),header = FALSE,stringsAsFactors = FALSE))
...
#Add a column to the data frame to identify the source of the data, in the case the source was from the testing
#Read in test activities from the y_test.txt file and use the plyr join function to match test activities to the activity names
...
#Create a data frame that holds the column names for the inertial data sets, this will be used later to name our columns 
...
in_tst_head= mutate(in_tst_head,totalaccelerationy= paste0(in_tst_head$X.totalaccelerationy.,in_tst_head$number))
...
features_test_data = data.table(read.table(paste0(path,"/X_test.txt"),header =FALSE, stringsAsFactors = FALSE))
...
features_headers_duplicate2 = which(features_headers == TRUE) #returns and integer of 0 so there is are no duplicate columns.
...
#Melt the data using the participantid,activity, and source columns as ids, and the measurements as variables
...
mean_by_activity_and_subject_narrow = melt(mean_by_activity_and_subject_wide, id =1:2,
                                           measure.vars = 3:length(wide_complete_data))
write.table(mean_by_activity_and_subject_narrow,file= paste0(path,"tidy_data.txt"),row.names = FALSE)